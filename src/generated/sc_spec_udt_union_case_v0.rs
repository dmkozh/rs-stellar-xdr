...
#[cfg(feature = "std")]
impl ReadXdrRc for ScSpecUdtUnionCaseV0 {
    fn read_xdr_with_buffer(r: &mut RcReader) -> Result<Self, Error> {
        r.with_limited_depth(|r| {
            let dv: ScSpecUdtUnionCaseV0Kind =
                <ScSpecUdtUnionCaseV0Kind as ReadXdrRc>::read_xdr_with_buffer(r)?;
            #[allow(clippy::match_same_arms, clippy::match_wildcard_for_single_variants)]
            let v = match dv {
                ScSpecUdtUnionCaseV0Kind::VoidV0 => {
                    Self::VoidV0(ScSpecUdtUnionCaseVoidV0::read_xdr_with_buffer(r)?)
                }
                ScSpecUdtUnionCaseV0Kind::TupleV0 => {
                    Self::TupleV0(ScSpecUdtUnionCaseTupleV0::read_xdr_with_buffer(r)?)
                }
                #[allow(unreachable_patterns)]
                _ => return Err(Error::Invalid),
            };
