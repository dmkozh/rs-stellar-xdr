...
#[cfg(feature = "std")]
impl ReadXdrRc for TxSetComponent {
    fn read_xdr_with_buffer(r: &mut RcReader) -> Result<Self, Error> {
        r.with_limited_depth(|r| {
            let dv: TxSetComponentType =
                <TxSetComponentType as ReadXdrRc>::read_xdr_with_buffer(r)?;
            #[allow(clippy::match_same_arms, clippy::match_wildcard_for_single_variants)]
            let v = match dv {
                TxSetComponentType::TxsetCompTxsMaybeDiscountedFee => {
                    Self::TxsetCompTxsMaybeDiscountedFee(
                        TxSetComponentTxsMaybeDiscountedFee::read_xdr_with_buffer(r)?,
                    )
                }
                #[allow(unreachable_patterns)]
                _ => return Err(Error::Invalid),
